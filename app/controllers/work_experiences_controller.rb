class WorkExperiencesController < ApplicationController
  ...
  def create
    if current_user.work_experience.nil?
      @work_experience = current_user.create_work_experience(work_experience_params)

      respond_to do |format|
        if @work_experience.save
          format.html { redirect_to statuses_url, notice: 'Work experience was successfully created.' }
          format.json { render :show, status: :created, location: @work_experience }
        else
          format.html { redirect_to edit_profile_url(current_user), alert: 'Work experience was not created'}
          format.json { render json: @work_experience.errors, status: :unprocessable_entity }
        end
      end
    else
      redirect_to edit_profile_url(current_user)
      flash[:alert] = 'Already exists Work experience'
    end
  end
  ...
  private
    ...
    def work_experience_params
      params.require(:work_experience).permit(:companyName, :designation, :location)
    end
end
